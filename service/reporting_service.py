...
def calculate_portfolio_daily_returns(data_dict: dict[str, pd.DataFrame], weights: dict[str, float]) -> pd.Series:
    ...
    logging.info(f"Aligned returns across assets. Computed {len(returns_df)} days of common returns.")
...
def calculate_portfolio_yield(data_dict: dict[str, pd.DataFrame], weights: dict[str, float]) -> float:
    ...
    for ticker, weight in weights.items():
        ...
        if df is not None and not df.empty and 'Dividends' in df.columns:
            ...
            if initial_price > 0 and n_days > 0:
                ...
                logging.info(f"Asset '{ticker}': Total Dividends={total_dividends:.2f}, Annualized Yield={asset_yield:.2%}")
...
def compute_metrics(returns: pd.Series, div_yield: float) -> dict[str, float]:
    ...
    logging.info(f"Computed Metrics: Ann. Return={annualized_return:.2%}, Ann. Volatility={annualized_volatility:.2%}, Ann. Yield={div_yield:.2%}")
