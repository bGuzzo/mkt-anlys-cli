...
\subsection{Unified USD Currency Normalization}
A major architectural feature of the system is the complete currency normalization to US Dollars (USD). For assets and benchmarks denominated in foreign currencies (e.g., EUR, JPY, GBP), daily close prices and dividend series are normalized to USD on a daily basis using historical daily FX rates fetched from yfinance (e.g., \texttt{EURUSD=X}). 
...
\begin{enumerate}
    ...
    \item \textbf{Currency Normalization Layer (\texttt{main\_service.py} \& \texttt{capm\_service.py}):} Normalizes historical Daily FX exchange rates to align asset/benchmark prices and dividends under a unified USD perspective.
...
\begin{itemize}
    ...
    \item \textbf{\texttt{tangency\_portfolio\_only.json}}: Summarizes the optimal tangency portfolio weights, return, volatility, Sharpe ratio, beta, alpha, and optional USD investment allocation details.
